...
#        super().compile_predictors(initialize)
    ...
    def build_autoencoders(self):
        ...
        # autoencoder = GanModel(inputs, output, discriminator, side)
        self.add_predictor('a', KerasModel(inputs[0], generator_a))
        self.add_predictor('b', KerasModel(inputs[1], generator_b))
        self.add_trainer('a', KerasModel(inputs[0], self.discriminator_a(inputs[0])))
        self.add_trainer('b', KerasModel(inputs[1], self.discriminator_b(inputs[1])))
    ...
    def encoder(self):
        ...
        n_filters = 32
        o = Conv2D(n_filters, 5, strides=(2,2), padding='same')(input_)
        n_filters *= 2
        o = Conv2D(n_filters, 5, strides=(2,2), padding='same')(o)
        n_filters *= 2
        o = Conv2D(n_filters, 5, strides=(2,2), padding='same')(o)
        n_filters *= 2
        # o = Conv2D(1024, 5, strides=(2,2), padding='same')(o)
        o = Flatten()(o)
        o = Dense(1024)(o)
...
class GanModel(KerasModel):
    def __init__(self, inputs, output, discriminator, side):
        ...
        self.combined = KerasModel(inputs, y)
    ...
    def compile(self, optimizer,
        ...
        self.combined.compile(optimizer, loss=loss, metrics=metrics, loss_weights=loss_weights,
            sample_weight_mode=sample_weight_mode, weighted_metrics=weighted_metrics,
            target_tensors=target_tensors)
        self.discriminator.trainable = True
        self.discriminator.compile(optimizer, loss=loss, metrics=metrics, loss_weights=loss_weights,
            sample_weight_mode=sample_weight_mode, weighted_metrics=weighted_metrics,
            target_tensors=target_tensors)
    ...
    def train_on_batch(self, x, y, **kwargs):
        ...
        gen = self.predict(x)
        dloss = self.discriminator.train_on_batch(y, valid)
        dloss += self.discriminator.train_on_batch(gen, invalid)
        dloss *= 0.5

        gloss = self.combined.train_on_batch(x, valid)
        logger.info('side %s dloss=%.5f gloss=%.5f' % (self.side, dloss, gloss))
        return (dloss + gloss) * 0.5
